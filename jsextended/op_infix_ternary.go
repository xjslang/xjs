...
func ParseInfixTernaryOp(p *parser.Parser, left ast.Expr) (node *InfixTernaryOp, err error) {
	node = &InfixTernaryOp{Cond: left}
	if node.Layout.QuestionMark, err = p.Expect(TERNARY); err != nil {
		return
	}
...
func PrintInfixTernaryOp(pr *printer.Printer, node *InfixTernaryOp) error {
	pr.Print(node.Cond)
