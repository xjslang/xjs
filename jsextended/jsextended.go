...
func ParserBuilder() *parser.Builder {
	...
	token.RegisterInfixOpType(PLUS_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(MINUS_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(MULTIPLY_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(DIVIDE_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(MODULO_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(OR_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(XOR_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(AND_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(EXPO_ASSIGN, token.ASSIGN.Precedence())
	...
	token.RegisterInfixOpType(SHL_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(SHR_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(USHR_ASSIGN, token.ASSIGN.Precedence())
	token.RegisterInfixOpType(STRICT_EQ, token.EQ.Precedence())
	token.RegisterInfixOpType(STRICT_NOT_EQ, token.EQ.Precedence())
	token.RegisterInfixOpType(OPTIONAL_CHAINING, token.DOT.Precedence())
	token.RegisterInfixOpType(INSTANCEOF, token.LT.Precedence())
	token.RegisterInfixOpType(ARROW, token.ASSIGN.Precedence()+1)
	token.RegisterInfixOpType(TERNARY, token.ASSIGN.Precedence()+1)
	token.RegisterInfixOpType(EXPO, token.MULTIPLY.Precedence()+1)
	token.RegisterInfixOpType(COALESCING, token.OR.Precedence())
	token.RegisterInfixOpType(OR_BITWISE, token.AND.Precedence()+1)
	token.RegisterInfixOpType(XOR_BITWISE, token.AND.Precedence()+2)
	token.RegisterInfixOpType(AND_BITWISE, token.AND.Precedence()+3)
	token.RegisterInfixOpType(SHL_BITWISE, token.LT.Precedence()+5)
	token.RegisterInfixOpType(SHR_BITWISE, token.LT.Precedence()+5)
	token.RegisterInfixOpType(USHR_BITWISE, token.LT.Precedence()+5)
	token.RegisterInfixOpType(IN, token.LT.Precedence())
	token.RegisterInfixOpType(token.STRING, token.LPAREN.Precedence())
