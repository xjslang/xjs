...
func ParseTryStmt(p *parser.Parser) (node *TryStmt, err error) {
	...
	if node.Try, err = ParseBlockStmt(p); err != nil {
		return
	}
	if p.CurrentToken.Type == CATCH {
		...
		if p.CurrentToken.Type == token.LPAREN {
			...
			p.AdvanceToken()
			if node.CatchParam, err = p.ParseExpr(); err != nil {
				return
			}
		...
		if node.Catch, err = ParseBlockStmt(p); err != nil {
			return
		}
	}
	if p.CurrentToken.Type == FINALLY {
		...
		p.AdvanceToken()
		if node.Finally, err = ParseBlockStmt(p); err != nil {
			return
		}
	}
